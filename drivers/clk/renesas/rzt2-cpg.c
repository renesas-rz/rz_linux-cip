...
static unsigned long rzt2n_cpg_pll_clk_recalc_rate(struct clk_hw *hw,
...
static const struct clk_ops rzt2_cpg_pll_ops = {
	.recalc_rate = rzt2n_cpg_pll_clk_recalc_rate,
};
...
static int rzt2n_cpg_reset(struct reset_controller_dev *rcdev,
				unsigned long id)
{
...
static int rzt2n_cpg_assert(struct reset_controller_dev *rcdev,
				unsigned long id)
{
	struct rzt2_cpg_priv *priv = rcdev_to_priv(rcdev);
	const struct rzt2_cpg_info *info = priv->info;
...
static int rzt2n_cpg_deassert(struct reset_controller_dev *rcdev,
				unsigned long id)
{
	struct rzt2_cpg_priv *priv = rcdev_to_priv(rcdev);
	const struct rzt2_cpg_info *info = priv->info;
...
static const struct reset_control_ops rzt2n_cpg_reset_ops = {
	.reset = rzt2n_cpg_reset,
	.assert = rzt2n_cpg_assert,
	.deassert = rzt2n_cpg_deassert,
};

static int rzt2n_cpg_reset_xlate(struct reset_controller_dev *rcdev,
...
static int rzt2n_cpg_reset_controller_register(struct rzt2_cpg_priv *priv)
{
	priv->rcdev.ops = &rzt2n_cpg_reset_ops;
	priv->rcdev.of_node = priv->dev->of_node;
	priv->rcdev.dev = priv->dev;
	priv->rcdev.of_reset_n_cells = 1;
	priv->rcdev.of_xlate = rzt2n_cpg_reset_xlate;
...
static int __init rzt2_cpg_probe(struct platform_device *pdev)
{
	...
	if (error)
		return error;
	error = rzt2n_cpg_reset_controller_register(priv);
	if (error)
		return error;
...
#ifdef CONFIG_CLK_R9A07G076
	{
		.compatible = "renesas,r9a07g076-cpg",
		.data = &r9a07g076_cpg_info,
	},
#endif
	{ /* sentinel */ }
};
