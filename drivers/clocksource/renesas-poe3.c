					 ...
					 enum mtu3_channel_port_output ch)
{
	...
	/* Clear OSTSTF */
	if (poe3->board_id == POE3_BOARD_RZT2) {
		...
		for (i = 0; i < num; i++) {
			...
			if (p->first_setup) {
				if (!strcmp(p->mtu_channel, "mtu3_ch0"))
					...
					if ((poe3->board_id == POE3_BOARD_RZT2) && active_level == 1) {
						switch (output_value) {
						case 0:
							alr1 |= ALR1_OLSG0AB;
							break;
...
static const struct renesas_poe3_soc_info poe3_info_rzt2 = {
	.board_id = POE3_BOARD_RZT2,
};
...
static const struct of_device_id renesas_poe3_of_table[] = {
	...
	{ .compatible = "renesas,r9a07g076-poe3", .data = &poe3_info_rzt2},
	{ },
};
