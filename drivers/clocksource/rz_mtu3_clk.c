...
static u32 rz_mtu3_clk_get_counter(struct rz_mtu3_clk_channel_priv *ch)
{
	...
	if (ch->chan->channel_number == RZ_MTU3_CHAN_8)
			...
			struct platform_device *pdev, struct rz_mtu3 *ddata)
{
	...
	mtu->rstc = devm_reset_control_get_optional_shared(pdev->dev.parent, NULL);
	...
	/* Start the index from 8 to prioritize channel 8 for clocksource
	 * and keep other channels for other functions
	 */
	for (i = RZ_MTU3_CHAN_8; i >= RZ_MTU3_CHAN_0; i--) {
