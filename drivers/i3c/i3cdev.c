			   ...
			   size_t count, loff_t *f_pos)
{
	...
	if (!ret)
		ret = copy_to_user(buf, tmp, xfers.len) ?
		      -EFAULT : xfers.len;
			       ...
			       unsigned int nxfers)
{
	...
	k_xfers = kcalloc(nxfers,
			  sizeof(*k_xfers) + sizeof(*data_ptrs),
			  GFP_KERNEL);
	if (!k_xfers)
		return -ENOMEM;
	...
	for (i = 0; i < nxfers; i++) {
		if (xfers[i].rnw &&
		    copy_to_user(u64_to_user_ptr(xfers[i].data),
...
i3cdev_get_ioc_priv_xfer(unsigned int cmd,
			 struct i3c_ioc_priv_xfer *u_xfers,
			 unsigned int *nxfers)
{
	u32 tmp = _IOC_SIZE(cmd);

	if (tmp % sizeof(struct i3c_ioc_priv_xfer))
...
static int i3cdev_ioc_priv_xfer(struct i3c_device *i3c,
				unsigned int cmd,
				struct i3c_ioc_priv_xfer *u_xfers)
{
...
static long i3cdev_ioctl(struct file *file,
			 unsigned int cmd, unsigned long arg)
{
	...
	dev_dbg(&i3c->dev, "ioctl cmd=0x%x arg=0x%lx\n", cmd, arg);
...
{
	file->private_data =
		container_of(inode->i_cdev, struct i3cdev_data, cdev);
	return 0;
}
