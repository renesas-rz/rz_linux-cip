				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	src_format = v4l2_subdev_get_try_format(sd, cfg, RZG2L_CSI2_SOURCE);
	...
	sink_format = v4l2_subdev_get_try_format(sd, cfg, RZG2L_CSI2_SINK);
				  ...
				  struct v4l2_subdev_pad_config *cfg)
{
	...
	return rzg2l_csi2_set_format(sd, cfg, &fmt);
}
				     ...
				     struct v4l2_mbus_frame_desc *fd)
{
	...
	remote_pad = media_entity_remote_pad(&csi2->pads[RZG2L_CSI2_SINK]);
...
static int rzg2l_csi2_parse_dt(struct rzg2l_csi2 *csi2)
{
	...
	asd = v4l2_async_notifier_add_fwnode_subdev(&csi2->notifier, fwnode,
						    sizeof(*asd));
	fwnode_handle_put(fwnode);
	if (IS_ERR(asd))
		return PTR_ERR(asd);

	ret = v4l2_async_subdev_notifier_register(&csi2->subdev, &csi2->notifier);
	if (ret)
