 ...
 * on RZ/G3S since they share the interrupts.
 */
...
static int rz_mtu3_probe(struct platform_device *pdev)
{
	...
	priv->rstc = devm_reset_control_get_optional_shared(&pdev->dev, NULL);
	if (IS_ERR(priv->rstc)) {
	...
	if (!priv->rstc)
		dev_warn(&pdev->dev, "no resets defined => skip reset control handling\n");
