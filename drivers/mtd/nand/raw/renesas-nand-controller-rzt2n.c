				      ...
				      const struct nand_interface_config *conf)
{
	...
	reg = FIELD_PREP(ASYNCTOGGLETIMING_TRH, trh_cnt);
	reg |= FIELD_PREP(ASYNCTOGGLETIMING_TRP, trp_cnt);
	reg |= FIELD_PREP(ASYNCTOGGLETIMING_TWH, twh_cnt);
	reg |= FIELD_PREP(ASYNCTOGGLETIMING_TWP, twp_cnt);
	...
	reg = DLLPHYCTRL_DLLRSTN;
	if (ext_wr_mode)
		reg |= DLLPHYCTRL_EXTENDWMD;
	if (ext_rd_mode)
		reg |= DLLPHYCTRL_EXTENDRMD;

	reg |= FIELD_PREP(DLLPHYCTRL_RESYNCWAITCNT, 7);
	reg |= FIELD_PREP(DLLPHYCTRL_RESYNCIDLECNT, 7);
	t->dll_phy_ctrl = reg;
