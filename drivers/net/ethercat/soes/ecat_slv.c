...
u8		MBX[MBXBUFFERS * MAX(MBXSIZE, MBXSIZEBOOT)];
struct _MBXcontrol	MBXcontrol[MBXBUFFERS];
struct _SMmap		SMmap2[MAX_MAPPINGS_SM2];
struct _SMmap		SMmap3[MAX_MAPPINGS_SM3];
struct _ESCvar		ESCvar;

/* Private variables */
static volatile int watchdog;

#if MAX_MAPPINGS_SM2 > 0
static u8 rxpdo[MAX_RXPDO_SIZE] __aligned(8);
#else
extern u8 rxpdo[];
#endif

#if MAX_MAPPINGS_SM3 > 0
static u8 txpdo[MAX_TXPDO_SIZE] __aligned(8);
#else
extern u8 txpdo[];
#endif
 ...
 */
u32 ESC_download_pre_objecthandler(u16 index,
				   u8 subindex,
				   void *data,
				   size_t size,
				   u16 flags)
{
	if (IS_RXPDO(index) || IS_TXPDO(index) ||
	    index == RX_PDO_OBJIDX || index == TX_PDO_OBJIDX) {
		u8 minSub = ((flags & COMPLETE_ACCESS_FLAG) == 0) ? 0 : 1;
 ...
 * @return SDO abort code, or 0 on success
 */
u32 ESC_download_post_objecthandler(u16 index, u8 subindex, u16 flags)
{
	if (ESCvar.post_object_download_hook)
 ...
 * @return SDO abort code, or 0 on success
 */
u32 ESC_upload_pre_objecthandler(u16 index,
				 u8 subindex,
				 void *data,
				 size_t *size,
				 u16 flags)
{
	if (ESCvar.pre_object_upload_hook)
 ...
 * @return SDO abort code, or 0 on success
 */
u32 ESC_upload_post_objecthandler(u16 index, u8 subindex, u16 flags)
{
	if (ESCvar.post_object_upload_hook)
 ...
 * event mask argument
 */
void ecat_slv_worker(u32 event_mask)
{
	do {
 ...
 */
void ecat_slv_init(struct esc_cfg_t *config)
{
