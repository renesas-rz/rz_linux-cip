...
void ESC_ALstatusgotoerror(u8 status, u16 errornumber)
{
	...
	as = (u8)(((status & ESCREG_AL_ERRACKMASK) << 4) | (as & 0x0f));
	/* Call post state change hook case it have been configured  */
	if (ESCvar.pre_state_change_hook)
	...
	an = status;
	/* Call post state change hook case it have been configured  */
	if (ESCvar.post_state_change_hook)
...
void ESC_SMack(u8 n)
{
	...
	ESC_read((u16)(ESCREG_SM0ACTIVATE + (n << 3)), &dummy, 1);
}
...
void ESC_SMstatus(u8 n)
{
	...
	sm = (struct _ESCsm2 *)&ESCvar.SM[n];
	ESC_read((u16)(ESCREG_SM0STATUS + (n << 3)), &sm->Status, 1);
}
 ...
 */
void ESC_SMwritepdi(u8 n)
{
	struct _ESCsm2 *sm;

	sm = (struct _ESCsm2 *)&ESCvar.SM[n];
	ESC_write((u16)(ESCREG_SM0PDI + (n << 3)), &sm->ActPDI, 1);
}
 ...
 */
void ESC_SMreadpdi(u8 n)
{
	struct _ESCsm2 *sm;

	sm = (struct _ESCsm2 *)&ESCvar.SM[n];
	ESC_read((u16)(ESCREG_SM0PDI + (n << 3)), &sm->ActPDI, 1);
}

/** Write 0 to Bit0 in SM PDI control register 0x807(+ offset to SyncManager n) to
 * Activate the Sync Manager n.
 *
 * @param[in] n   = Write to Sync Manager no. n
 */
void ESC_SMenable(u8 n)
{
	struct _ESCsm2 *sm;

	sm = (struct _ESCsm2 *)&ESCvar.SM[n];
	sm->ActPDI &= (u8)~ESCREG_SMENABLE_BIT;
	ESC_SMwritepdi(n);
...
/** Write 1 to Bit0 in SM PDI control register 0x807(+ offset to SyncManager n) to
 * De-activte the Sync Manager n.
 *
 * @param[in] n   = Write to Sync Manager no. n
 */
void ESC_SMdisable(u8 n)
{
	struct _ESCsm2 *sm;
 ...
 */
u16 ESC_checkDC(void)
{
	u16 ret = 0;

	u8 sync_act = ESC_SYNCactivation();
	...
	if ((sync_act & (ESCREG_SYNC_ACT_ACTIVATED | ESCREG_SYNC_AUTO_ACTIVATED)) > 0) {
		...
		if (ESCvar.esc_check_dc_handler)
...
u8 ESC_checkmbx(u8 state)
{
	...
	if ((etohs(SM->PSA) != ESC_MBX0_sma) || (etohs(SM->Length) != ESC_MBX0_sml) ||
		...
		ESC_SMdisable(0);
		ESC_SMdisable(1);
		return (u8)(ESCinit | ESCerror);      //fail state change
	}
	SM = (struct _ESCsm2 *)&ESCvar.SM[1];
...
/** Try to start bootstrap mailboxes for current ALControl state request by enabling
 * SyncManager 0 and 1.
 * If all mailbox settings is correct we return incoming state request, otherwise
 * we return state Init with Error flag set and update local ALerror with code 0x16 Invalid
 * mailbox configuration.
...
{
	struct _MBX *MB = (struct _MBX *)&MBX[0];
	u16 length;
	...
	if (length > (ESC_MBX0_sml - ESC_MBXHSIZE))
		length = (u16)(ESC_MBX0_sml - ESC_MBXHSIZE);

	ESC_read((u16)(ESC_MBX0_sma + ESC_MBXHSIZE), MB->b, length);
 ...
 */
void ESC_writembx(u8 n)
{
	struct _MBXh *MBh = (struct _MBXh *)&MBX[n * ESC_MBXSIZE];
	u8 dummy = 0;
	u16 length;
	...
	if (length > (ESC_MBX1_sml - ESC_MBXHSIZE))
		length = (u16)(ESC_MBX1_sml - ESC_MBXHSIZE);

	ESC_write(ESC_MBX1_sma, MBh, (u16)(ESC_MBXHSIZE + length));
...
u8 ESC_claimbuffer(void)
{
	...
	if (n) {
		...
		MBh = (struct _MBXh *)&MBX[n * ESC_MBXSIZE];
		ESCvar.mbxcnt++;
...
{
	u8 MBXout;
	struct _MBXerr *mbxerr;

	MBXout = ESC_claimbuffer();
	if (MBXout) {
		mbxerr = (struct _MBXerr *)&MBX[MBXout * ESC_MBXSIZE];
		mbxerr->mbxheader.length = htoes((u16)0x04);
		mbxerr->mbxheader.mbxtype = MBXERR;
		mbxerr->type = htoes((u16)0x01);
...
{
	u8 mbxhandle = 0;
	struct _MBXh *MBh = (struct _MBXh *)&MBX[0];
 ...
 * @return = incoming state request if every thing checks out OK. = state(PREOP | ERROR)
 * if something isn't correct.
 */
u8 ESC_checkSM23(u8 state)
{
	struct _ESCsm2 *SM;

	ESC_read(ESCREG_SM2, (void *)&ESCvar.SM[2], sizeof(ESCvar.SM[2]));
	SM = (struct _ESCsm2 *)&ESCvar.SM[2];
 ...
 *
 * @param[in] state   = Requested state.
 * @return = state, incoming state request if every thing checks out OK. =state(PREOP | ERROR)
 * if something isn't correct.
 */
u8 ESC_startinput(u8 state)
{
	...
	if (state != (ESCpreop | ESCerror)) {
		...
		if (dc_check_result > 0) {
			...
			if (ESCvar.esc_hw_interrupt_enable) {
				u32 int_mask;
...
void ESC_stopinput(void)
{
	...
	if (ESCvar.use_interrupt != 0 &&
	    ESCvar.esc_hw_interrupt_disable)
...
static bool ESC_check_id_request(u16 ALcontrol, u8 *an)
{
	if ((ALcontrol & ESCREG_AL_ID_REQUEST) != 0) {
		u8 state = ALcontrol & ESCREG_AL_ERRACKMASK;
...
static u8 ESC_load_device_id(void)
{
	u16 device_id;

	if (ESCvar.get_device_id) {
...
#ifdef ESC_DEBUG
static char *ESC_state_to_string(u8 ESC_state)
{
	switch (ESC_state) {
...
{
	static struct sm_cfg_t mb0 = {MBX0_sma, MBX0_sml, MBX0_sme, MBX0_smc, 0};
	static struct sm_cfg_t mb1 = {MBX1_sma, MBX1_sml, MBX1_sme, MBX1_smc, 0};
	static struct sm_cfg_t mbboot0 = {MBX0_sma_b, MBX0_sml_b, MBX0_sme_b, MBX0_smc_b, 0};
	static struct sm_cfg_t mbboot1 = {MBX1_sma_b, MBX1_sml_b, MBX1_sme_b, MBX1_smc_b, 0};
