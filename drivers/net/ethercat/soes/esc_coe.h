...
struct _objectlist {
	...
	const struct _objd *objdesc;
};

struct _SMmap {
	const struct _objd *obj;

	const struct _objectlist *objectlistitem;
	u32 offset;
};
...
#define COMPLETE_ACCESS_FLAG    BIT(15)

void ESC_coeprocess(void);
s16 SDO_findsubindex(s32 nidx, u8 subindex);
s32 SDO_findobject(u16 index);
u16 sizeOfPDO(u16 index, int *nmappings, struct _SMmap *sm, int max_mappings);
void COE_initDefaultValues(void);

void COE_pdoPack(u8 *buffer, int nmappings, struct _SMmap *sm);
void COE_pdoUnpack(u8 *buffer, int nmappings, struct _SMmap *sm);
u8 COE_maxSub(u16 index);

extern u32 ESC_download_post_objecthandler(u16 index, u8 subindex, u16 flags);
extern u32 ESC_download_pre_objecthandler(u16 index,
					  u8 subindex,
					  void *data,
					  size_t size,
					  u16 flags);
extern u32 ESC_upload_pre_objecthandler(u16 index,
					u8 subindex,
					void *data,
					size_t *size,
					u16 flags);
extern u32 ESC_upload_post_objecthandler(u16 index, u8 subindex, u16 flags);
extern const struct _objectlist SDOobjects[];
