...
#define EOE_MAKEU32(a, b, c, d)	(((u32)((a) & 0xff) << 24) | \
				((u32)((b) & 0xff) << 16) | \
				((u32)((c) & 0xff) << 8)  | \
				(u32)((d) & 0xff))

/** Get one byte from the 4-byte address */
#define eoe_ip4_addr1(ipaddr) (((const u8 *)(&(ipaddr)->addr))[0])
#define eoe_ip4_addr2(ipaddr) (((const u8 *)(&(ipaddr)->addr))[1])
#define eoe_ip4_addr3(ipaddr) (((const u8 *)(&(ipaddr)->addr))[2])
#define eoe_ip4_addr4(ipaddr) (((const u8 *)(&(ipaddr)->addr))[3])
...
/** IPv4 address length */
#define EOE_IP4_LENGTH        4U /* sizeof(u32) */
...
struct eoe_ethaddr_t {
	u8 addr[EOE_ETHADDR_LENGTH];
} CC_PACKED;
CC_PACKED_END

struct _EOEvar {
	/** Pointer to current RX buffer to fill */
	struct eoe_pbuf_t rxebuf;
	/** Pointer to current TX buff to Send */
	struct eoe_pbuf_t txebuf;
...
struct eoe_param_t {
	...
	u8 ip_set:1;
	u8 subnet_set:1;
	u8 default_gateway_set:1;
	u8 dns_ip_set:1;
	u8 dns_name_set:1;
	struct eoe_ethaddr_t mac;
	struct eoe_ip4_addr_t ip;
	struct eoe_ip4_addr_t subnet;
	struct eoe_ip4_addr_t default_gateway;
	struct eoe_ip4_addr_t dns_ip;
 ...
 */
static void EOE_ip_u32o_byte(struct eoe_ip4_addr_t *ip, u8 *byte_ip)
{
 ...
 */
static void EOE_ip_byte_to_uint32(u8 *byte_ip, struct eoe_ip4_addr_t *ip)
{
...
int EOE_ecat_get_gateway(u8 port, u32 *default_gateway)
{
	...
	if (port < EOE_NUMBER_OF_PORTS) {
		...
		if (nic_ports[port_ix].default_gateway_set) {
			*default_gateway = EOE_NTOHL(nic_ports[port_ix].default_gateway.addr);
			ret = 0;
		}
	}
	return ret;
...
int EOE_ecat_set_gateway(u8 port, u32 default_gateway)
{
	...
	if (port < EOE_NUMBER_OF_PORTS) {
		port_ix = EOE_PORT_INDEX(port);
		nic_ports[port_ix].default_gateway.addr = EOE_HTONL(default_gateway);
 ...
 */
static void EOE_no_data_response(u16 frameinfo1, u16 result)
{
	struct _EOE *eoembx;
	u8 mbxhandle;
	...
	if (mbxhandle) {
		eoembx = (struct _EOE *)&MBX[mbxhandle * ESC_MBXSIZE];
...
static void EOE_get_ip(void)
{
	...
	if (mbxhandle) {
		...
		if (nic_ports[port_ix].ip_set) {
			...
			EOE_ip_u32o_byte(&nic_ports[port_ix].ip,
		...
		if (nic_ports[port_ix].subnet_set) {
			...
			EOE_ip_u32o_byte(&nic_ports[port_ix].subnet,
					 &eoembx->data[data_offset]);
			/* Add size of uint32 IP address */
			data_offset += EOE_IP4_LENGTH;
		}
		...
		if (nic_ports[port_ix].default_gateway_set) {
			...
			EOE_ip_u32o_byte(&nic_ports[port_ix].default_gateway,
					 &eoembx->data[data_offset]);
			/* Add size of uint32 IP address */
			data_offset += EOE_IP4_LENGTH;
		}
		...
		if (nic_ports[port_ix].dns_ip_set) {
			...
			EOE_ip_u32o_byte(&nic_ports[port_ix].dns_ip,
					 &eoembx->data[data_offset]);
			/* Add size of uint32 IP address */
			data_offset += EOE_IP4_LENGTH;
		}
...
static void EOE_set_ip(void)
{
	...
	if (flags & EOE_PARAM_DNS_NAME_INCLUDE) {
		u32 dns_len = MIN((eoedatasize - data_offset), EOE_DNS_NAME_LENGTH);
	...
	if (data_offset > eoedatasize) {
		 ...
		 */
		if (eoe_cfg->store_ethernet_settings)
			result = (u16)eoe_cfg->store_ethernet_settings();
		else
...
static void EOE_receive_fragment(void)
{
	struct _EOE *eoembx;
	u32 eoedatasize;
	u16 frameinfo1, frameinfo2;

	eoembx = (struct _EOE *)&MBX[0];
	eoedatasize = etohs(eoembx->mbxheader.length) - ESC_EOEHSIZE;
	frameinfo1 = etohs(eoembx->eoeheader.frameinfo1);
	frameinfo2 = etohs(eoembx->eoeheader.frameinfo2);
	...
	if (EOEvar.rxfragmentno == 0) {
		...
		u32 offset = (EOE_HDR_FRAME_OFFSET_GET(frameinfo2) << 5);
	...
	if ((EOEvar.rxframeoffset + eoedatasize) <= EOEvar.rxframesize) {
		memcpy((u8 *)(EOEvar.rxebuf.payload + EOEvar.rxframeoffset),
		       eoembx->data,
		       eoedatasize);
 ...
 */
static void EOE_send_fragment(void)
{
	struct _EOE *eoembx;
	u8 mbxhandle;
	int len;
	...
	if (mbxhandle) {
		...
		if ((len_to_send + ESC_EOEHSIZE + ESC_MBXHSIZE) > ESC_MBXSIZE)
			...
			len_to_send = (((ESC_MBXSIZE - ESC_EOEHSIZE - ESC_MBXHSIZE) >> 5) << 5);
		...
		if (eoe_cfg->fragment_sent_event)
...
void ESC_eoeprocess(void)
{
	...
	if (ESCvar.xoe == MBXEOE) {
		...
		if (etohs(eoembx->mbxheader.length) < ESC_EOEHSIZE) {
			EOE_no_data_response(EOE_INIT_RESP | EOE_HDR_LAST_FRAGMENT,
					     MBXERR_SIZETOOSHORT);
		} else {
			frameinfo1 = etohs(eoembx->eoeheader.frameinfo1);
