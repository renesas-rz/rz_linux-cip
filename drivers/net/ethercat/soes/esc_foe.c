...
//define if FOE_readshould be supported
 ...
 * password
 */
static u32 FOE_fopen(char *name, u8 num_chars, u32 pass, u8 op)
{
	u32 i;
	...
	for (i = 0; i < foe_cfg->n_files; i++) {
		if (strncmp(foe_file_name, foe_cfg->files[i].name, num_chars) == 0) {
 ...
 */
static u32 FOE_fread(u8 *data, u32 maxlength)
{
	u32 ncopied = 0;
 ...
 * @return Number of copied bytes.
 */
static u32 FOE_fwrite(u8 *data, u32 length)
{
	u32 ncopied = 0;
	u32 failed = 0;
	...
	while (length && (FOEvar.fend - FOEvar.fposition) && !failed) {
		...
		if (FOEvar.fbufposition >= foe_cfg->buffer_size) {
			failed = foe_file->write_function(foe_file,
							  foe_cfg->fbuffer,
							  FOEvar.fbufposition);
		...
		FOEvar.fposition++;
		if (failed)
			DPRINT("Failed FOE_fwritencopied=%" PRIu32 "\n", ncopied);
		else
			ncopied++;
	}
	...
	DPRINT("FOE_fwriteEND with : %" PRIu32 "\n", ncopied);
	return ncopied;
}

/** Function handling the final FOE_fwritewhen we close up regardless
 * if we have filled the buffers or not.
 *
 * @return Number of copied bytes on success, 0= if failed.
 */
static u32 FOE_fclose(void)
{
	u32 failed = 0;
 ...
 */
static void FOE_abort(u32 code)
{
	struct _FOE *foembx;
	u8 mbxhandle;
	...
	if (code) {
		...
		if (mbxhandle) {
			foembx = (struct _FOE *)&MBX[mbxhandle * ESC_MBXSIZE];
			/* Don't bother with error text for now. */
 ...
 */
static u32 FOE_send_data_packet(void)
{
	struct _FOE *foembx;
	u32 data_len;
	u8 mbxhandle;
...
static void FOE_read(void)
{
	...
	FOE_init();
	foembx = (struct _FOE *)&MBX[0];
	/* Get the length of the file name in octets. */
	data_len = (u8)(etohs(foembx->mbxheader.length) - ESC_FOEHSIZE);
 ...
 *
 */
static void FOE_data(void)
{
	struct _FOE *foembx;
	u32 packet;
	u32 data_len, ncopied;
	u32 res;
	...
	if (packet != FOEvar.foepacket) {
		DPRINT("FOE_datapacket error, packet: %" PRIu32 ", foeheader.packet: %" PRIu32 "\n",
		       packet,
		       FOEvar.foepacket);
		FOE_abort(FOE_ERR_PACKETNO);
	} else if (data_len == 0) {
		DPRINT("FOE_datacompleted\n");
		FOE_fclose();
		res = FOE_send_ack();
		FOE_init();
	} else if (FOEvar.fposition + data_len > FOEvar.fend) {
		DPRINT("FOE_datadisk full\n");
		FOE_abort(FOE_ERR_DISKFULL);
	} else {
		ncopied = FOE_fwrite(foembx->data, data_len);
		if (!ncopied) {
			DPRINT("FOE_datano copied\n");
			FOE_abort(FOE_ERR_PROGERROR);
		} else if (data_len == ESC_FOE_DATA_SIZE) {
			DPRINT("FOE_datadata_len == FOE_DATA_SIZE\n");
			if (ncopied != data_len) {
				DPRINT("FOE_dataonly %" PRIu32 " of %" PRIu32 " copied\n",
				       ncopied,
				       data_len);
				FOE_abort(FOE_ERR_PROGERROR);
			}
			...
			if (ncopied != data_len || FOE_fclose()) {
				DPRINT("FOE_fclosefailed to write extra buffer\n");
				FOE_abort(FOE_ERR_PROGERROR);
			} else {
				DPRINT("FOE_datacompleted\n");
				res = FOE_send_ack();
