...
/* Renesas Layer 3 Ethernet Switch Module (ESWM) device driver
 * Based on Ethernet Switch2 driver from Linux kernel v6.11-rc2
 *
...
static void eswm_reset(struct eswm_private *priv)
{
	iowrite32(RRC_RR, priv->addr + RRC);
	iowrite32(RRC_RR_CLR, priv->addr + RRC);
}

static void eswm_clock_enable(struct eswm_private *priv)
{
	iowrite32(RCEC_ACE_DEFAULT | RCEC_RCE, priv->addr + RCEC);
}

static void eswm_clock_disable(struct eswm_private *priv)
{
	iowrite32(RCDC_RCD, priv->addr + RCDC);
}

static bool eswm_agent_clock_is_enabled(void __iomem *coma_addr,
					   unsigned int port)
{
...
static void eswm_agent_clock_ctrl(void __iomem *coma_addr, unsigned int port,
				     int enable)
{
	u32 val;
...
static int eswm_bpool_config(struct eswm_private *priv)
{
	u32 val;
	...
	return eswm_reg_wait(priv->addr, CABPIRM, CABPIRM_BPR, CABPIRM_BPR);
}

static void eswm_coma_init(struct eswm_private *priv)
{
	iowrite32(CABPPFLC_INIT_VALUE, priv->addr + CABPPFLC0);
}

/* R-Switch-2 block (TOP) */
static void eswm_top_init(struct eswm_private *priv)
{
	unsigned int i;

	for (i = 0; i < ESWM_MAX_NUM_QUEUES; i++)
		iowrite32((i / 16) << (GWCA_INDEX * 8), priv->addr + TPEMIMC7(i));
}

/* Forwarding engine block (MFWD) */
static void eswm_fwd_init(struct eswm_private *priv)
{
	unsigned int i;

	/* For ETHA */
	for (i = 0; i < ESWM_NUM_HW; i++) {
	...
	iowrite32(GENMASK(ESWM_NUM_PORTS - 1, 0), priv->addr + FWPBFC(priv->gwca.index));
}

/* Gateway CPU agent block (GWCA) */
static int eswm_gwca_change_mode(struct eswm_private *priv,
				    enum eswm_gwca_mode mode)
{
	int ret;

	if (!eswm_agent_clock_is_enabled(priv->addr, priv->gwca.index))
		eswm_agent_clock_ctrl(priv->addr, priv->gwca.index, 1);

	iowrite32(mode, priv->addr + GWMC);

	ret = eswm_reg_wait(priv->addr, GWMS, GWMS_OPS_MASK, mode);

	if (mode == GWMC_OPC_DISABLE)
		eswm_agent_clock_ctrl(priv->addr, priv->gwca.index, 0);

	return ret;
}

static int eswm_gwca_mcast_table_reset(struct eswm_private *priv)
{
	iowrite32(GWMTIRM_MTIOG, priv->addr + GWMTIRM);

	return eswm_reg_wait(priv->addr, GWMTIRM, GWMTIRM_MTR, GWMTIRM_MTR);
}

static int eswm_gwca_axi_ram_reset(struct eswm_private *priv)
{
	iowrite32(GWARIRM_ARIOG, priv->addr + GWARIRM);

	return eswm_reg_wait(priv->addr, GWARIRM, GWARIRM_ARR, GWARIRM_ARR);
}

static bool eswm_is_any_data_irq(struct eswm_private *priv, u32 *dis, bool tx)
{
	u32 *mask = tx ? priv->gwca.tx_irq_bits : priv->gwca.rx_irq_bits;
	unsigned int i;

	for (i = 0; i < ESWM_NUM_IRQ_REGS; i++) {
		if (dis[i] & mask[i])
			return true;
	}
...
static void eswm_get_data_irq_status(struct eswm_private *priv, u32 *dis)
{
	unsigned int i;
...
static void eswm_ack_data_irq(struct eswm_private *priv,
				 unsigned int index)
{
...
static bool eswm_is_queue_rxed(struct eswm_gwca_queue *gq)
{
	struct eswm_ext_ts_desc *desc = &gq->rx_ring[gq->dirty];
					   ...
					   unsigned int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (gq->rx_bufs[index])
			continue;
		gq->rx_bufs[index] = netdev_alloc_frag(ESWM_BUF_SIZE);
		if (!gq->rx_bufs[index])
			goto err;
	}
...
static void eswm_gwca_queue_free(struct net_device *ndev,
				    struct eswm_gwca_queue *gq)
{
	...
	if (!gq->dir_tx) {
		dma_free_coherent(ndev->dev.parent,
				  sizeof(struct eswm_ext_ts_desc) *
...
static void eswm_gwca_ts_queue_free(struct eswm_private *priv)
{
	struct eswm_gwca_queue *gq = &priv->gwca.ts_queue;

	dma_free_coherent(&priv->pdev->dev,
			  sizeof(struct eswm_ts_desc) * (gq->ring_size + 1),
			  gq->ts_ring, gq->ring_dma);
	gq->ts_ring = NULL;
}

static int eswm_gwca_queue_alloc(struct net_device *ndev,
				    ...
				    bool dir_tx, unsigned int ring_size)
{
	...
	if (!dir_tx) {
		...
		if (!gq->rx_bufs)
			return -ENOMEM;
		if (eswm_gwca_queue_alloc_rx_buf(gq, 0, gq->ring_size) < 0)
			goto out;
...
static int eswm_gwca_queue_format(struct net_device *ndev,
				     ...
				     struct eswm_gwca_queue *gq)
{
	unsigned int ring_size = sizeof(struct eswm_ext_desc) * gq->ring_size;
	struct eswm_ext_desc *desc;
	struct eswm_desc *linkfix;
	dma_addr_t dma_addr;
	unsigned int i;
	...
	eswm_desc_set_dptr(&desc->desc, gq->ring_dma);
	desc->desc.die_dt = DT_LINKFIX;

	linkfix = &priv->gwca.linkfix_table[gq->index];
	linkfix->die_dt = DT_LINKFIX;
	eswm_desc_set_dptr(linkfix, gq->ring_dma);
...
static void eswm_gwca_ts_queue_fill(struct eswm_private *priv,
				       unsigned int start_index,
				       unsigned int num)
{
	struct eswm_gwca_queue *gq = &priv->gwca.ts_queue;
	struct eswm_ts_desc *desc;
	unsigned int i, index;
...
static int eswm_gwca_queue_ext_ts_format(struct net_device *ndev,
					    ...
					    struct eswm_gwca_queue *gq)
{
	unsigned int ring_size = sizeof(struct eswm_ext_ts_desc) * gq->ring_size;
	struct eswm_ext_ts_desc *desc;
	struct eswm_desc *linkfix;
	int err;

	memset(gq->rx_ring, 0, ring_size);
	err = eswm_gwca_queue_ext_ts_fill(ndev, gq, 0, gq->ring_size);
	if (err < 0)
		return err;
...
static int eswm_gwca_linkfix_alloc(struct eswm_private *priv)
{
	unsigned int i, num_queues = priv->gwca.num_queues;
	struct eswm_gwca *gwca = &priv->gwca;
	struct device *dev = &priv->pdev->dev;

	gwca->linkfix_table_size = sizeof(struct eswm_desc) * num_queues;
...
static void eswm_gwca_linkfix_free(struct eswm_private *priv)
{
	struct eswm_gwca *gwca = &priv->gwca;
...
static int eswm_gwca_ts_queue_alloc(struct eswm_private *priv)
{
	struct eswm_gwca_queue *gq = &priv->gwca.ts_queue;
	struct eswm_ts_desc *desc;
	...
	gq->ts_ring = dma_alloc_coherent(&priv->pdev->dev,
					 sizeof(struct eswm_ts_desc) *
					 (gq->ring_size + 1), &gq->ring_dma, GFP_KERNEL);

	if (!gq->ts_ring)
		return -ENOMEM;

	eswm_gwca_ts_queue_fill(priv, 0, TS_RING_SIZE);
...
static struct eswm_gwca_queue *eswm_gwca_get(struct eswm_private *priv)
{
	struct eswm_gwca_queue *gq;
	unsigned int index;
...
static void eswm_gwca_put(struct eswm_private *priv,
			     struct eswm_gwca_queue *gq)
{
	clear_bit(gq->index, priv->gwca.used);
}

static int eswm_txdmac_alloc(struct net_device *ndev)
{
	struct eswm_device *rdev = netdev_priv(ndev);
...
static int eswm_rxdmac_alloc(struct net_device *ndev)
{
	...
	rdev->rx_queue = eswm_gwca_get(priv);
	if (!rdev->rx_queue)
		return -EBUSY;
...
static int eswm_rxdmac_init(struct eswm_private *priv, unsigned int index)
{
	struct eswm_device *rdev = priv->rdev[index];
	struct net_device *ndev = rdev->ndev;

	return eswm_gwca_queue_ext_ts_format(ndev, priv, rdev->rx_queue);
}

static int eswm_gwca_hw_init(struct eswm_private *priv)
{
	unsigned int i;
	int err;

	err = eswm_gwca_change_mode(priv, GWMC_OPC_DISABLE);
	if (err < 0)
		return err;
	err = eswm_gwca_change_mode(priv, GWMC_OPC_CONFIG);
	if (err < 0)
		return err;

	err = eswm_gwca_mcast_table_reset(priv);
	if (err < 0)
		return err;
	err = eswm_gwca_axi_ram_reset(priv);
	if (err < 0)
		return err;
	...
	for (i = 0; i < ESWM_NUM_PORTS; i++) {
		err = eswm_rxdmac_init(priv, i);
		if (err < 0)
			return err;
		err = eswm_txdmac_init(priv, i);
		if (err < 0)
			return err;
	}
	...
	if (err < 0)
		return err;
	return eswm_gwca_change_mode(priv, GWMC_OPC_OPERATION);
}

static int eswm_gwca_hw_deinit(struct eswm_private *priv)
{
	int err;
	...
	if (err < 0)
		return err;
	err = eswm_gwca_change_mode(priv, GWMC_OPC_RESET);
	if (err < 0)
		return err;
...
static int eswm_gwca_halt(struct eswm_private *priv)
{
	...
	priv->gwca_halt = true;
	err = eswm_gwca_hw_deinit(priv);
	dev_err(&priv->pdev->dev, "halted (%d)\n", err);

	return err;
}

static struct sk_buff *eswm_rx_handle_desc(struct net_device *ndev,
					      struct eswm_gwca_queue *gq,
					      struct eswm_ext_ts_desc *desc)
{
	dma_addr_t dma_addr = eswm_desc_get_dptr(&desc->desc);
	u16 pkt_len = le16_to_cpu(desc->desc.info_ds) & RX_DS;
	u8 die_dt = desc->desc.die_dt & DT_MASK;
	struct sk_buff *skb = NULL;

	dma_unmap_single(ndev->dev.parent, dma_addr, ESWM_MAP_BUF_SIZE,
			 DMA_FROM_DEVICE);
	...
	case DT_FSTART:
	case DT_FSINGLE:
		skb = build_skb(gq->rx_bufs[gq->cur], ESWM_BUF_SIZE);
		if (skb) {
			skb_reserve(skb, ESWM_HEADROOM);
			skb_put(skb, pkt_len);
			gq->pkt_len = pkt_len;
			if (die_dt == DT_FSTART) {
				gq->skb_fstart = skb;
	...
	case DT_FEND:
		skb_add_rx_frag(gq->skb_fstart, skb_shinfo(gq->skb_fstart)->nr_frags,
				...
				offset_in_page(gq->rx_bufs[gq->cur]) + ESWM_HEADROOM,
				pkt_len, ESWM_BUF_SIZE);
		if (die_dt == DT_FEND) {
			skb = gq->skb_fstart;
			gq->skb_fstart = NULL;
		}
...
static bool eswm_rx(struct net_device *ndev, int *quota)
{
	struct eswm_device *rdev = netdev_priv(ndev);
	...
	while ((desc->desc.die_dt & DT_MASK) != DT_FEMPTY) {
		dma_rmb();
		skb = eswm_rx_handle_desc(ndev, gq, desc);
		if (!skb)
			goto out;
...
out:
	...
	ret = eswm_gwca_queue_alloc_rx_buf(gq, gq->dirty, num);
	if (ret < 0)
		goto err;
...
static int eswm_poll(struct napi_struct *napi, int budget)
{
	struct net_device *ndev = napi->dev;
	struct eswm_private *priv;
	struct eswm_device *rdev;
	unsigned long flags;
	int quota = budget;
...
static void eswm_queue_interrupt(struct net_device *ndev)
{
	struct eswm_device *rdev = netdev_priv(ndev);
...
static irqreturn_t eswm_data_irq(struct eswm_private *priv, u32 *dis)
{
	struct eswm_gwca_queue *gq;
	...
	for (i = 0; i < priv->gwca.num_queues; i++) {
		...
		eswm_ack_data_irq(priv, gq->index);
		eswm_queue_interrupt(gq->ndev);
	}

	return IRQ_HANDLED;
}

static irqreturn_t eswm_gwca_irq(int irq, void *dev_id)
{
	struct eswm_private *priv = dev_id;
	u32 dis[ESWM_NUM_IRQ_REGS];
	irqreturn_t ret = IRQ_NONE;

	eswm_get_data_irq_status(priv, dis);

	if (eswm_is_any_data_irq(priv, dis, true) ||
	    eswm_is_any_data_irq(priv, dis, false))
		ret = eswm_data_irq(priv, dis);

	return ret;
}

static int eswm_gwca_request_irqs(struct eswm_private *priv)
{
	...
	for (i = 0; i < GWCA_NUM_IRQS; i++) {
		...
		ret = devm_request_irq(&priv->pdev->dev, irq, eswm_gwca_irq,
				       0, irq_name, priv);
		if (ret < 0)
			return ret;
	}
...
static void eswm_ts(struct eswm_private *priv)
{
	struct eswm_gwca_queue *gq = &priv->gwca.ts_queue;
	struct eswm_gwca_ts_info *ts_info, *ts_info2;
	struct skb_shared_hwtstamps shhwtstamps;
	struct eswm_ts_desc *desc;
	struct timespec64 ts;
	unsigned int num;
	...
	num = eswm_get_num_cur_queues(gq);
	eswm_gwca_ts_queue_fill(priv, gq->dirty, num);
	gq->dirty = eswm_next_queue_index(gq, false, num);
}

static irqreturn_t eswm_gwca_ts_irq(int irq, void *dev_id)
{
	struct eswm_private *priv = dev_id;
...
static int eswm_gwca_ts_request_irqs(struct eswm_private *priv)
{
	int irq;
	...
	return devm_request_irq(&priv->pdev->dev, irq, eswm_gwca_ts_irq,
				    ...
				    enum eswm_etha_mode mode)
{
	...
	if (mode == EAMC_OPC_DISABLE)
		eswm_agent_clock_ctrl(etha->coma_addr, etha->index, 0);
...
static void eswm_rmac_setting(struct eswm_etha *etha, const u8 *mac)
{
	...
	eswm_modify(etha->addr, MPIC, MPIC_PIS_MASK | MPIC_LSC_MASK,
...
static struct device_node *eswm_get_port_node(struct eswm_device *rdev)
{
...
static int eswm_etha_get_params(struct eswm_device *rdev)
{
	u32 max_speed;
	int err;
...
static void eswm_mii_unregister(struct eswm_device *rdev)
{
	if (rdev->etha->mii) {
...
static int eswm_ether_port_init_one(struct eswm_device *rdev)
{
	...
	err = eswm_mii_register(rdev);
	if (err < 0)
		return err;

	err = eswm_phy_device_init(rdev);
	if (err < 0) {

		eswm_mii_unregister(rdev);
		return err;
	}
...
static void eswm_ether_port_deinit_one(struct eswm_device *rdev)
{
	eswm_phy_device_deinit(rdev);
	eswm_mii_unregister(rdev);
}

static int eswm_ether_port_init_all(struct eswm_private *priv)
{
	unsigned int i;
	int err;

	eswm_for_each_enabled_port(priv, i) {
		err = eswm_ether_port_init_one(priv->rdev[i]);
		if (err)
			goto err_init_one;
	}
...
err_init_one:
	eswm_for_each_enabled_port_continue_reverse(priv, i)
		eswm_ether_port_deinit_one(priv->rdev[i]);

	return err;
}

static void eswm_ether_port_deinit_all(struct eswm_private *priv)
{
	unsigned int i;
...
static int eswm_open(struct net_device *ndev)
{
	struct eswm_device *rdev = netdev_priv(ndev);
	unsigned long flags;
	...
	if (bitmap_empty(rdev->priv->opened_ports, ESWM_NUM_PORTS))
...
static int eswm_stop(struct net_device *ndev)
{
	struct eswm_device *rdev = netdev_priv(ndev);
	struct eswm_gwca_ts_info *ts_info, *ts_info2;
	unsigned long flags;
...
static bool eswm_ext_desc_set_info1(struct eswm_device *rdev,
				       struct sk_buff *skb,
				       struct eswm_ext_desc *desc)
{
...
static bool eswm_ext_desc_set(struct eswm_device *rdev,
				 struct sk_buff *skb,
				 struct eswm_ext_desc *desc,
				 dma_addr_t dma_addr, u16 len, u8 die_dt)
{
	...
	if (!eswm_ext_desc_set_info1(rdev, skb, desc))
		return false;
...
static u8 eswm_ext_desc_get_die_dt(unsigned int nr_desc, unsigned int index)
{
	if (nr_desc == 1)
...
static u16 eswm_ext_desc_get_len(u8 die_dt, unsigned int orig_len)
{
	switch (die_dt & DT_MASK) {
	case DT_FSINGLE:
	case DT_FEND:
		return (orig_len % ESWM_DESC_BUF_SIZE) ?: ESWM_DESC_BUF_SIZE;
	case DT_FSTART:
	case DT_FMID:
		return ESWM_DESC_BUF_SIZE;
	default:
		return 0;
	}
}
...
static netdev_tx_t eswm_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	for (i = nr_desc; i-- > 0; ) {
		desc = &gq->tx_ring[eswm_next_queue_index(gq, true, i)];
		die_dt = eswm_ext_desc_get_die_dt(nr_desc, i);
		dma_addr = dma_addr_orig + i * ESWM_DESC_BUF_SIZE;
		len = eswm_ext_desc_get_len(die_dt, skb->len);
		if (!eswm_ext_desc_set(rdev, skb, desc, dma_addr, len, die_dt))
			goto err_unmap;
	}

	wmb();	/* gq->cur must be incremented after die_dt was set */

	gq->cur = eswm_next_queue_index(gq, true, nr_desc);
...
static struct net_device_stats *eswm_get_stats(struct net_device *ndev)
{
	return &ndev->stats;
}

static int eswm_hwstamp_get(struct net_device *ndev, struct ifreq *req)
{
	struct eswm_device *rdev = netdev_priv(ndev);
	...
	config.tx_type = ptp_priv->tstamp_tx_ctrl ? HWTSTAMP_TX_ON :
						    HWTSTAMP_TX_OFF;
	switch (ptp_priv->tstamp_rx_ctrl & ESWM_RXTSTAMP_TYPE) {
	case ESWM_RXTSTAMP_TYPE_V2_L2_EVENT:
		config.rx_filter = HWTSTAMP_FILTER_PTP_V2_L2_EVENT;
		break;
	case ESWM_RXTSTAMP_TYPE_ALL:
...
static int eswm_hwstamp_set(struct net_device *ndev, struct ifreq *req)
{
	struct eswm_device *rdev = netdev_priv(ndev);
	u32 tstamp_rx_ctrl = ESWM_RXTSTAMP_ENABLED;
	struct hwtstamp_config config;
	u32 tstamp_tx_ctrl;
	...
	case HWTSTAMP_TX_OFF:
		...
		break;
	case HWTSTAMP_TX_ON:
		tstamp_tx_ctrl = ESWM_TXTSTAMP_ENABLED;
		break;
	default:
		return -ERANGE;
	}
	...
	case HWTSTAMP_FILTER_PTP_V2_L2_EVENT:
		tstamp_rx_ctrl |= ESWM_RXTSTAMP_TYPE_V2_L2_EVENT;
		break;
	default:
		config.rx_filter = HWTSTAMP_FILTER_ALL;
		tstamp_rx_ctrl |= ESWM_RXTSTAMP_TYPE_ALL;
		break;
	}
...
static int eswm_eth_ioctl(struct net_device *ndev, struct ifreq *req, int cmd)
{
	...
	switch (cmd) {
	case SIOCGHWTSTAMP:
		return eswm_hwstamp_get(ndev, req);
	case SIOCSHWTSTAMP:
		return eswm_hwstamp_set(ndev, req);
	default:
...
static const struct ethtool_ops eswm_ethtool_ops = {
...
static const struct of_device_id renesas_eth_sw_of_table[] = {
	{ .compatible = "renesas,r9a07g076-ether-switch", },
	{ }
};
MODULE_DEVICE_TABLE(of, renesas_eth_sw_of_table);

static void eswm_etha_init(struct eswm_private *priv, unsigned int index)
{
	struct eswm_etha *etha = &priv->etha[index];

	memset(etha, 0, sizeof(*etha));
	etha->index = index;
	etha->addr = priv->addr + ESWM_ETHA_OFFSET + index * ESWM_ETHA_SIZE;
...
static int eswm_device_alloc(struct eswm_private *priv, unsigned int index)
{
	...
	err = eswm_rxdmac_alloc(ndev);
	if (err < 0)
		goto out_rxdmac;

	err = eswm_txdmac_alloc(ndev);
	if (err < 0)
		goto out_txdmac;
...
static void eswm_device_free(struct eswm_private *priv, unsigned int index)
{
	struct eswm_device *rdev = priv->rdev[index];
	struct net_device *ndev = rdev->ndev;
...
static int eswm_init(struct eswm_private *priv)
{
	unsigned int i;
	int err;
	...
	eswm_clock_enable(priv);
	eswm_top_init(priv);
	err = eswm_bpool_config(priv);
	if (err < 0)
		return err;

	eswm_coma_init(priv);

	err = eswm_gwca_linkfix_alloc(priv);
	if (err < 0)
		return -ENOMEM;

	err = eswm_gwca_ts_queue_alloc(priv);
	if (err < 0)
		goto err_ts_queue_alloc;

	for (i = 0; i < ESWM_NUM_PORTS; i++) {
		err = eswm_device_alloc(priv, i);
		if (err < 0) {
			for (; i-- > 0; )
	...
	err = eswm_ptp_register(priv->ptp_priv, ESWM_PTP_REG_LAYOUT,
				     ESWM_PTP_CLOCK);
	if (err < 0)
		goto err_ptp_register;

	err = eswm_gwca_request_irqs(priv);
	if (err < 0)
		goto err_gwca_request_irq;

	err = eswm_gwca_ts_request_irqs(priv);
	if (err < 0)
		goto err_gwca_ts_request_irq;

	err = eswm_gwca_hw_init(priv);
	if (err < 0)
		goto err_gwca_hw_init;

	err = eswm_ether_port_init_all(priv);
	if (err)
...
err_register_netdev:
	eswm_ether_port_deinit_all(priv);

err_ether_port_init_all:
	eswm_gwca_hw_deinit(priv);

err_gwca_hw_init:
err_gwca_ts_request_irq:
err_gwca_request_irq:
	eswm_ptp_unregister(priv->ptp_priv);
...
err_device_alloc:
	eswm_gwca_ts_queue_free(priv);

err_ts_queue_alloc:
	eswm_gwca_linkfix_free(priv);

	return err;
}

static const struct soc_device_attribute eswm_soc_no_speed_change[]  = {
	{ .soc_id = "r9a07g076" },
	{ /* Sentinel */ }
};
...
static void eswm_deinit(struct eswm_private *priv)
{
	unsigned int i;
	...
	eswm_for_each_enabled_port(priv, i) {
		struct eswm_device *rdev = priv->rdev[i];

		unregister_netdev(rdev->ndev);
		eswm_ether_port_deinit_one(rdev);
	}
	...
	eswm_clock_disable(priv);
}

static void renesas_eth_sw_remove(struct platform_device *pdev)
{
	struct eswm_private *priv = platform_get_drvdata(pdev);
...
{
	struct eswm_private *priv = dev_get_drvdata(dev);
	struct net_device *ndev;
	unsigned int i;
