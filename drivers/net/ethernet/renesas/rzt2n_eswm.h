...
#define eswm_for_each_enabled_port_continue_reverse(priv, i)	\
		for (; i-- > 0; )					\
			if (priv->rdev[i]->disabled)			\
				continue;				\
			else
...
#define TS_RING_SIZE		(TX_RING_SIZE * ESWM_NUM_PORTS)

#define ESWM_MAX_MTU		9600
#define ESWM_HEADROOM	(NET_SKB_PAD + NET_IP_ALIGN)
#define ESWM_DESC_BUF_SIZE	2048
#define ESWM_TAILROOM	SKB_DATA_ALIGN(sizeof(struct skb_shared_info))
#define ESWM_ALIGN		128
#define ESWM_BUF_SIZE	(ESWM_HEADROOM + ESWM_DESC_BUF_SIZE + \
				 ESWM_TAILROOM + ESWM_ALIGN)
#define ESWM_MAP_BUF_SIZE	(ESWM_BUF_SIZE - ESWM_HEADROOM)
#define ESWM_MAX_CTAG_PCP	7

#define ESWM_TIMEOUT_US		100000

#define ESWM_TOP_OFFSET		0x00008000
#define ESWM_COMA_OFFSET	0x00009000
#define ESWM_ETHA_OFFSET	0x0000a000	/* with RMAC */
#define ESWM_ETHA_SIZE		0x00002000	/* with RMAC */
#define ESWM_GWCA0_OFFSET	0x0000e000

/* TODO: hardcoded ETHA/GWCA settings for now */
#define GWCA_IRQ_RESOURCE_NAME	"gwca0_rxtx%d"
#define GWCA_IRQ_NAME		"eswm: gwca0_rxtx%d"
#define GWCA_NUM_IRQS		8
#define GWCA_INDEX		0
#define AGENT_INDEX_GWCA	2
#define GWCA_IPV_NUM		0
#define GWRO			ESWM_GWCA0_OFFSET

#define GWCA_TS_IRQ_RESOURCE_NAME	"gwca0_rxts0"
#define GWCA_TS_IRQ_NAME		"eswm: gwca0_rxts0"
...
#define MPSM_MFF_C22		0
#define MPSM_MFF_C45		1
...
#define RRC_RR			BIT(0)
#define RRC_RR_CLR		0
#define	RCEC_ACE_DEFAULT	GENMASK((ESWM_NUM_HW-1), 0)
...
struct eswm_ts_desc {
	struct eswm_desc desc;
	__le32 ts_nsec;
	__le32 ts_sec;
} __packed;

struct eswm_ext_desc {
	struct eswm_desc desc;
	__le64 info1;
} __packed;

struct eswm_ext_ts_desc {
	struct eswm_desc desc;
	__le64 info1;
	__le32 ts_nsec;
	__le32 ts_sec;
 ...
 */
struct eswm_gwca_queue {
	union {
		struct eswm_ext_desc *tx_ring;
		struct eswm_ext_ts_desc *rx_ring;
		struct eswm_ts_desc *ts_ring;
	};
...
#define ESWM_NUM_IRQ_REGS	(ESWM_MAX_NUM_QUEUES / BITS_PER_TYPE(u32))
struct eswm_gwca {
	unsigned int index;
	struct eswm_desc *linkfix_table;
	dma_addr_t linkfix_table_dma;
	u32 linkfix_table_size;
	struct eswm_gwca_queue *queues;
	int num_queues;
	struct eswm_gwca_queue ts_queue;
	struct list_head ts_info_list;
	DECLARE_BITMAP(used, ESWM_MAX_NUM_QUEUES);
	u32 tx_irq_bits[ESWM_NUM_IRQ_REGS];
	u32 rx_irq_bits[ESWM_NUM_IRQ_REGS];
	int speed;
};
...
struct eswm_device {
	...
	void __iomem *addr;
	struct eswm_gwca_queue *tx_queue;
	struct eswm_gwca_queue *rx_queue;
	u8 ts_tag;
	bool disabled;
...
struct eswm_mfwd {
	struct eswm_mac_table_entry *mac_table_entries;
...
struct eswm_private {
	...
	struct eswm_gwca gwca;
	struct eswm_etha etha[ESWM_NUM_PORTS];
	struct eswm_mfwd mfwd;
