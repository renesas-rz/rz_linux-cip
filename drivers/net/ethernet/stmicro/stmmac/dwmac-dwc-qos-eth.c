...
static int renesas_rzt2_eqos_pcs_init(struct stmmac_priv *priv)
{
	struct renesas_rzt2_eqos *eqos = priv->plat->bsp_priv;
...
static void renesas_rzt2_eqos_pcs_exit(struct stmmac_priv *priv)
{
	if (priv->hw->phylink_pcs)
		ethss_destroy(priv->hw->phylink_pcs);
}

static struct phylink_pcs *renesas_rzt2_eqos_select_pcs(struct stmmac_priv *priv,
							phy_interface_t interface)
{
	return priv->hw->phylink_pcs;
}

static int renesas_rzt2_eqos_init(struct platform_device *pdev, void *priv)
{
	struct renesas_rzt2_eqos *eqos = priv;
	unsigned long rate;
	u32 value;
...
static int renesas_rzt2_eqos_probe(struct platform_device *pdev,
				struct plat_stmmacenet_data *data,
				struct stmmac_resources *res)
{
	struct device *dev = &pdev->dev;
	struct device_node *node = pdev->dev.of_node;
	struct renesas_rzt2_eqos *eqos;
...
bypass_clk_reset_gpio:
	data->init = renesas_rzt2_eqos_init;
	data->bsp_priv = eqos;
	data->pcs_init = renesas_rzt2_eqos_pcs_init;
	data->pcs_exit = renesas_rzt2_eqos_pcs_exit;
	data->select_pcs = renesas_rzt2_eqos_select_pcs;
	data->flags |= STMMAC_FLAG_SPH_DISABLE;

	err = renesas_rzt2_eqos_init(pdev, eqos);
	if (err < 0)
		goto reset;
...
static void renesas_rzt2_eqos_remove(struct platform_device *pdev)
{
	struct renesas_rzt2_eqos *eqos = get_stmmac_bsp_priv(&pdev->dev);
...
static const struct dwc_eth_dwmac_data renesas_rzt2_eqos_data = {
	.probe = renesas_rzt2_eqos_probe,
	.remove = renesas_rzt2_eqos_remove,
};
...
static const struct of_device_id dwc_eth_dwmac_match[] = {
	...
	{ .compatible = "renesas,rzt2-eqos", .data = &renesas_rzt2_eqos_data },
	{ }
};
