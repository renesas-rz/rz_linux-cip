...
/*
 * PCIe DMA engine for PCIe on Renesas RZ/T2N Series SoCs
 *
...
static inline struct rzt2_pcie_dma_chan *to_rzt2_pcie_dma_chan(struct dma_chan *c)
{
	return container_of(c, struct rzt2_pcie_dma_chan, vc.chan);
};

static inline struct rzt2_pcie_dma_desc *to_rzt2_pcie_dma_desc(struct virt_dma_desc *vd)
{
	return container_of(vd, struct rzt2_pcie_dma_desc, vd);
};

static int rzt2_pcie_dma_xfer_desc(struct rzt2_pcie_dma_chan *chan)
{
	struct rzt2_pcie *pcie = chan->dmac->pcie;
	...
	chan->desc = to_rzt2_pcie_dma_desc(vdesc);
	...
	rzt2_pci_write_reg(pcie, lower_32_bits(dsa), DMA_DESCRIPTORL_ADDR_LOWER_REG(chan->index));
	rzt2_pci_write_reg(pcie, upper_32_bits(dsa), DMA_DESCRIPTORL_ADDR_UPPER_REG(chan->index));

	/* Set QUE Entry */
	rzt2_pci_write_reg(pcie, QUE_INTERRUPT_ON, QUE_ENTRY_REG(chan->index));
	/* Start xfer */
	rzt2_pci_write_reg(pcie, DMA_QUE_EN, DMA_CHANNEL_CONTROL_REG(chan->index));

	return 0;
}

static struct rzt2_pcie_dma_desc *rzt2_pcie_dma_alloc_desc(struct rzt2_pcie_dma_chan *chan,
							     int sg_len)
{
	struct rzt2_pcie_dma_desc *desc;
	int i;
...
static void rzt2_pcie_dma_release_desc(struct rzt2_pcie_dma_desc *desc)
{
	struct rzt2_pcie_dma_chan *chan = desc->chan;
	int i = 0;
...
static int rzt2_pcie_dma_alloc_chan_resources(struct dma_chan *ch)
{
	struct rzt2_pcie_dma_chan *chan = to_rzt2_pcie_dma_chan(ch);

	/* Create the dma pool for descriptor allocation */
	chan->pool = dma_pool_create(dev_name(&ch->dev->device),
					    chan->dmac->dev,
					    sizeof(struct rzt2_pcie_dma_hw_node),
					    __alignof__(struct rzt2_pcie_dma_hw_node),
					    0);
...
static void rzt2_pcie_dma_free_chan_resources(struct dma_chan *ch)
{
	struct rzt2_pcie_dma_chan *chan = to_rzt2_pcie_dma_chan(ch);
	unsigned long flags;
...
static size_t rzt2_pcie_dma_get_residue(struct rzt2_pcie_dma_chan *chan,
					 struct virt_dma_desc *vdesc,
					 bool in_progress)
{
	struct rzt2_pcie_dma_desc *desc = chan->desc;
	struct rzt2_pcie *pcie = chan->dmac->pcie;
	size_t residue = 0;
	u32 reg;
	int i;
	...
	if (in_progress) {
		reg = rzt2_pci_read_reg(pcie, DMA_REST_SIZE_REG(chan->index));
		residue += reg;
	} else {
...
static enum dma_status rzt2_pcie_dma_tx_status(struct dma_chan *ch,
						dma_cookie_t cookie,
						struct dma_tx_state *txstate)
{
	...
	if (chan->desc && cookie == chan->desc->vd.tx.cookie)
		txstate->residue = rzt2_pcie_dma_get_residue(chan, vd, true);
	else if (vd)
		txstate->residue = rzt2_pcie_dma_get_residue(chan, vd, false);
	else
		txstate->residue = 0;
...
rzt2_pcie_dma_prep_slave_sg(struct dma_chan *ch, struct scatterlist *sgl,
			     unsigned int sg_len,
			     enum dma_transfer_direction direction,
			     unsigned long flags, void *context)
{
	struct rzt2_pcie_dma_chan *chan = to_rzt2_pcie_dma_chan(ch);
	struct rzt2_pcie_dma_hw_node *hw_node;
	struct rzt2_pcie_dma_desc *desc;
	...
	desc = rzt2_pcie_dma_alloc_desc(chan, sg_len);
	if (!desc) {
...
err:
	...
	rzt2_pcie_dma_release_desc(desc);

	return NULL;
}

static int rzt2_pcie_dma_terminate_all(struct dma_chan *ch)
{
	struct rzt2_pcie_dma_chan *chan = to_rzt2_pcie_dma_chan(ch);
	struct rzt2_pcie *pcie = chan->dmac->pcie;
	unsigned long flags;
	...
	rzt2_pci_write_reg(pcie, DMA_QUE_CLR, DMA_CHANNEL_CONTROL_REG(chan->index));
...
static void rzt2_pcie_dma_issue_pending(struct dma_chan *ch)
{
	struct rzt2_pcie_dma_chan *chan = to_rzt2_pcie_dma_chan(ch);
	unsigned long flags;
	...
	if (vchan_issue_pending(&chan->vc) && !chan->desc)
		if (rzt2_pcie_dma_xfer_desc(chan) < 0)
...
static int rzt2_pcie_dma_config(struct dma_chan *ch,
...
static int rzt2_pcie_dma_pause(struct dma_chan *ch)
{
	struct rzt2_pcie_dma_chan *chan = to_rzt2_pcie_dma_chan(ch);
	struct rzt2_pcie *pcie = chan->dmac->pcie;
	u32 reg;

	reg = rzt2_pci_read_reg(pcie, DMA_CHANNEL_CONTROL_REG(chan->index));
	reg &= (~DMA_QUE_EN);
	rzt2_pci_write_reg(pcie, reg, DMA_CHANNEL_CONTROL_REG(chan->index));

	return 0;
}

static void rzt2_pcie_dma_free_desc(struct virt_dma_desc *vdesc)
{
	struct rzt2_pcie_dma_desc *desc = to_rzt2_pcie_dma_desc(vdesc);
	int i;
...
static irqreturn_t rzt2_pcie_dma_irq_handler(int irq, void *dev_id)
{
	struct rzt2_pcie *pcie = dev_id;
	struct rzt2_pcie_dmac *dmac = &pcie->dmac;
	...
	for (i = 0; i < dmac->n_channels; i++) {
		...
		if (!dma_status) {
			...
			continue;
		} else if ((dma_status & RZT2_PCIE_DMA_END) && !(dma_status & RZT2_PCIE_DMA_STOP))
			chan->status = DMA_COMPLETE;
		else if (dma_status & RZT2_PCIE_DMA_ERR)
			chan->status = DMA_ERROR;
		else if (dma_status & RZT2_PCIE_DMA_STOP)
			chan->status = (dma_status & RZT2_PCIE_DMA_END) ?
							DMA_COMPLETE : DMA_PAUSED;

		rzt2_rmw(pcie, DMA_INTERRUPT_STATUS_REG,
...
static irqreturn_t rzt2_pcie_dma_irq_handler_thread(int irq, void *dev_id)
{
	struct rzt2_pcie *pcie = dev_id;
	struct rzt2_pcie_dmac *dmac = &pcie->dmac;
	struct rzt2_pcie_dma_chan *chan;
	...
	for (i = 0; i < dmac->n_channels; i++) {
		...
		if (chan->status == DMA_IN_PROGRESS)
			...
			dev_info(pcie->dev, "DMA_REST_SIZE_REG = 0x%08x\n",
				rzt2_pci_read_reg(pcie, DMA_REST_SIZE_REG(i)));
			dev_info(pcie->dev, "AXI_REQUEST_ADDR_LOWER_REG = 0x%08x\n",
				rzt2_pci_read_reg(pcie, AXI_REQUEST_ADDR_LOWER_REG(i)));
			dev_info(pcie->dev, "AXI_REQUEST_ADDR_UPPER_REG = 0x%08x\n",
				rzt2_pci_read_reg(pcie, AXI_REQUEST_ADDR_UPPER_REG(i)));
			dev_info(pcie->dev, "PCIE_REQUEST_ADDR_LOWER_REG = 0x%08x\n",
				rzt2_pci_read_reg(pcie, PCIE_REQUEST_ADDR_LOWER_REG(i)));
			dev_info(pcie->dev, "PCIE_REQUEST_ADDR_UPPER_REG = 0x%08x\n",
				rzt2_pci_read_reg(pcie, PCIE_REQUEST_ADDR_UPPER_REG(i)));
			dev_info(pcie->dev, "DMAC_ERROR_STATUS_REG = 0x%08x\n",
				rzt2_pci_read_reg(pcie, DMAC_ERROR_STATUS_REG(i)));
		}
	}
...
int rzt2_pcie_dma_init(struct rzt2_pcie *pcie)
{
	struct rzt2_pcie_dmac *dmac = &pcie->dmac;
	...
	ret = devm_request_threaded_irq(dev, dmac->dma_irq, rzt2_pcie_dma_irq_handler,
					rzt2_pcie_dma_irq_handler_thread,
	...
	dmac->channels = devm_kcalloc(dev, dmac->n_channels,
				   sizeof(struct rzt2_pcie_dma_chan), GFP_KERNEL);
	if (!dmac->channels) {
		ret = -ENOMEM;
		goto err;
	}
	...
	for (i = 0; i < dmac->n_channels; i++) {
		struct rzt2_pcie_dma_chan *chan = &dmac->channels[i];

		chan->dmac = dmac;
		chan->index = i;
		chan->desc = NULL;
		chan->vc.desc_free = rzt2_pcie_dma_free_desc;
	...
	engine->device_alloc_chan_resources = rzt2_pcie_dma_alloc_chan_resources;
	engine->device_free_chan_resources = rzt2_pcie_dma_free_chan_resources;
	engine->device_tx_status = rzt2_pcie_dma_tx_status;
	engine->device_prep_slave_sg = rzt2_pcie_dma_prep_slave_sg;
	engine->device_config = rzt2_pcie_dma_config;
	engine->device_terminate_all = rzt2_pcie_dma_terminate_all;
	engine->device_issue_pending = rzt2_pcie_dma_issue_pending;
	engine->device_pause = rzt2_pcie_dma_pause;
...
void rzt2_pcie_dma_remove(struct rzt2_pcie *pcie)
{
	struct rzt2_pcie_dmac *dmac = &pcie->dmac;
	int i;
	...
	for (i = 0; i < dmac->n_channels; i++) {
		...
		if (!chan->desc)
			rzt2_pcie_dma_release_desc(chan->desc);
	}
}
