...
/*
 * PCIe driver for Renesas RZ/T2N Series SoCs
...
#define DMA_INTERRUPT_STATUS_REG				0x080C
	#define RZT2_PCIE_DMA_END					BIT(0)
	#define RZT2_PCIE_DMA_STOP					BIT(1)
	#define RZT2_PCIE_DMA_QUE_EMP				BIT(2)
	#define RZT2_PCIE_DMA_ERR					BIT(3)
	#define RZT2_PCIE_DMA_STATUS_ALL			(RZT2_PCIE_DMA_END | \
								RZT2_PCIE_DMA_STOP | \
								RZT2_PCIE_DMA_QUE_EMP | \
								RZT2_PCIE_DMA_ERR)
...
#define RZT2_WINDOW_SIZE_MIN					0x00001000
#define RZT2_WINDOW_SIZE_MAX					0xFFFFFFFF
#define RZT2_PCI_WINDOW_SIZE_MAX				0x40000000

#define INT_PCI_MSI_NR	32
#define INT_PCI_INTX_NR	1

#define RZT2_PCI_MAX_RESOURCES		2
...
#define PCIE_CONF_BAR_CTRL_AS_MEM	0x0
#define PCIE_CONF_BAR_CTRL_64BITS	0x4
#define PCIE_CONF_BAR_CTRL_PREFETCH	0x8
...
struct rzt2_axi_window_set {
	u32	base[RZT2_PCI_MAX_RESOURCES];
	u32	base_u[RZT2_PCI_MAX_RESOURCES];
	u32	mask[RZT2_PCI_MAX_RESOURCES];
	u32	dest[RZT2_PCI_MAX_RESOURCES];
	u32	dest_u[RZT2_PCI_MAX_RESOURCES];
};

struct rzt2_pci_window_set {
	u32	base[RZT2_PCI_MAX_RESOURCES];
	u32	mask[RZT2_PCI_MAX_RESOURCES];
	u32	dest_u[RZT2_PCI_MAX_RESOURCES];
	u32	dest_l[RZT2_PCI_MAX_RESOURCES];
};

struct rzt2_interrupt_set {
	u32	msi_win_addrl;
	u32	msi_win_addru;
	u32	msi_win_mask;
	u32	intx_ena;
...
struct rzt2_save_reg {
	struct rzt2_axi_window_set		axi_window;
	struct rzt2_pci_window_set		pci_window;
	struct rzt2_interrupt_set		interrupt;
};

#ifdef CONFIG_RZT2_PCIE_DMA
struct rzt2_pcie_dma_hw_node {
	u32 param0;
	u32 param1;
	u32 size;
	u32 padding;
...
struct rzt2_pcie_dma_sw_node {
	dma_addr_t pdesc;
	struct rzt2_pcie_dma_hw_node *desc;
};

struct rzt2_pcie_dma_desc {
	struct virt_dma_desc vd;
	struct rzt2_pcie_dma_chan *chan;
	dma_addr_t dev_addr;
	enum dma_transfer_direction direction;
	unsigned int n_nodes;
	struct rzt2_pcie_dma_sw_node node[];
};

struct rzt2_pcie_dma_chan {
	struct virt_dma_chan vc;
	struct rzt2_pcie_dmac *dmac;
...
struct rzt2_pcie_dmac {
	struct dma_device engine;
	struct rzt2_pcie *pcie;
	struct device *dev;
	int dma_irq;
	unsigned int n_channels;
	struct rzt2_pcie_dma_chan *channels;
...
struct rzt2_pcie {
	struct device			*dev;
	void __iomem			*base;
	struct rzt2_save_reg	save_reg;
#ifdef CONFIG_RZT2_PCIE_DMA
	struct rzt2_pcie_dmac	dmac;
#endif
};

void rzt2_pci_write_reg(struct rzt2_pcie *pcie, u32 val, unsigned long reg);
u32 rzt2_pci_read_reg(struct rzt2_pcie *pcie, unsigned long reg);
void rzt2_rmw(struct rzt2_pcie *pcie, int where, u32 mask, u32 data);
u32 rzt2_read_conf(struct rzt2_pcie *pcie, int where);
void rzt2_write_conf(struct rzt2_pcie *pcie, u32 data, int where);
void rzt2_phy_write(struct rzt2_pcie *pcie, u32 data, int where);
void rzt2_pcie_set_outbound(struct rzt2_pcie *pcie, int win,
			    struct resource_entry *window);
void rzt2_pcie_set_inbound(struct rzt2_pcie *pcie, u64 cpu_addr,
			   u64 pci_addr, u64 flags, int idx, bool host);

u32 rzt2_read_conf_ep(struct rzt2_pcie *pcie, int where, u8 fn);
void rzt2_write_conf_ep(struct rzt2_pcie *pcie, u32 data, int where, u8 fn);
void rzt2_pcie_ep_set_outbound(struct rzt2_pcie *pcie, int win, struct resource_entry *window,
			       phys_addr_t cpu_addr, u64 pci_addr, size_t size);
void rzt2_pcie_ep_set_inbound(struct rzt2_pcie *pcie, u64 cpu_addr, u64 pci_addr,
