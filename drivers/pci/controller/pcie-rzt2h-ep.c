...
static int PCIE_CFG_Initialize_ep(struct rzt2h_pcie *pcie)
{
	...
	/* Base Address Register Mask00 (Lower) (Function #1): PCIE Configuration Registers Adr 60A0h */
	rzt2h_write_conf_ep(pcie, BASEADR_MKL_ALLM, PCI_EP_BARMSK00L_F, 0);

	/* Base Address Register Mask00 (Upper) (Function #1): PCIE Configuration Registers Adr 60A4h */
...
static void PCIE_EP_INT_Initialize(struct rzt2h_pcie *pcie)
{
	...
	rzt2h_pci_write_reg(pcie, INT_MR_CLR, PCI_EP_MSGRCVIS_REG);		/* Set PCI_RC 0124h */

	/* Set Message Receive Interrupt Enable */
	rzt2h_pci_write_reg(pcie, PCI_EP_MSGRCVIE, PCI_EP_MSGRCVIE_REG);	/* Set PCI_RC 0120h */
}
...
static int rzt2h_pcie_hw_init_ep(struct rzt2h_pcie *pcie)
{
	...
	rzt2h_pci_write_reg(pcie, value | RST_PS_B | RST_GP_B | RST_B | RST_OUT_B,  PCI_EP_RESET_REG);
...
static int rzt2h_pcie_ep_write_header(struct pci_epc *epc, u8 fn,
...
static int rzt2h_pcie_ep_set_bar(struct pci_epc *epc, u8 func_no,
...
static void rzt2h_pcie_ep_clear_bar(struct pci_epc *epc, u8 fn,
				   struct pci_epf_bar *epf_bar)
{
	struct rzt2h_pcie_endpoint *ep = epc_get_drvdata(epc);
	enum pci_barno bar = epf_bar->barno;
...
static int rzt2h_pcie_ep_set_msi(struct pci_epc *epc, u8 fn, u8 interrupts)
{
	struct rzt2h_pcie_endpoint *ep = epc_get_drvdata(epc);
	struct rzt2h_pcie *pcie = &ep->pcie;
	u32 flags;
...
static int rzt2h_pcie_ep_get_msi(struct pci_epc *epc, u8 fn)
{
	struct rzt2h_pcie_endpoint *ep = epc_get_drvdata(epc);
	struct rzt2h_pcie *pcie = &ep->pcie;
	u32 flags;
...
static int rzt2h_pcie_ep_map_addr(struct pci_epc *epc, u8 fn,
...
static void rzt2h_pcie_ep_unmap_addr(struct pci_epc *epc, u8 fn,
				    phys_addr_t addr)
{
	struct rzt2h_pcie_endpoint *ep = epc_get_drvdata(epc);
	struct resource_entry win;
...
static int rzt2h_pcie_ep_raise_irq(struct pci_epc *epc, u8 fn,
...
rzt2h_pcie_ep_get_features(struct pci_epc *epc, u8 func_no)
{
...
static int rzt2h_pcie_ep_remove(struct platform_device *pdev)
{
	return -EBUSY;
}
