				...
				phys_addr_t cpu_addr, u64 pci_addr, size_t size)
{
	...
	rzv2h_pci_write_reg(pcie, (u32)(cpu_addr >> 32), PCIE_WINDOW_BASEU_REG(win));
	rzv2h_rmw(pcie, PCIE_WINDOW_BASEL_REG(win), 0xFFFFF000, (u32)(cpu_addr & 0xFFFFF000));

	/* PW0 mask: PCIE_WINDOW_MASKU_REG: 0x110C PCIE_WINDOW_MASKL_REG: 0x1108 */
	rzv2h_pci_write_reg(pcie, (u64)(mask >> 32), PCIE_WINDOW_MASKU_REG(win));
	rzv2h_pci_write_reg(pcie, (u64)(mask & 0xFFFFF000), PCIE_WINDOW_MASKL_REG(win));

	/* PD0 addr: PCIE_DESTINATION_HI_REG: 0x1114 PCIE_DESTINATION_LO_REG: 0x1110 */
	rzv2h_pci_write_reg(pcie, (u32)(pci_addr >> 32), PCIE_DESTINATION_HI_REG(win));
	rzv2h_pci_write_reg(pcie, (u32)(pci_addr & 0xFFFFFFFF), PCIE_DESTINATION_LO_REG(win));
			   ...
			   u64 pci_addr, u64 flags, int idx, u8 fn)
{
	...
	rzv2h_pci_write_reg(pcie, lower_32_bits(pci_addr),
					AXI_WINDOW_BASEL_REG(idx) + function_offset);
	rzv2h_pci_write_reg(pcie, upper_32_bits(pci_addr),
					AXI_WINDOW_BASEU_REG(idx) + function_offset);
	rzv2h_pci_write_reg(pcie, lower_32_bits(cpu_addr),
					AXI_DESTINATIONL_REG(idx) + function_offset);
	rzv2h_pci_write_reg(pcie, upper_32_bits(cpu_addr),
					AXI_DESTINATIONU_REG(idx) + function_offset);
	rzv2h_pci_write_reg(pcie, lower_32_bits(flags),
					AXI_WINDOW_MASKL_REG(idx) + function_offset);
	rzv2h_pci_write_reg(pcie, upper_32_bits(flags),
					AXI_WINDOW_MASKU_REG(idx) + function_offset);
	rzv2h_rmw(pcie, AXI_WINDOW_BASEL_REG(idx) + function_offset,
