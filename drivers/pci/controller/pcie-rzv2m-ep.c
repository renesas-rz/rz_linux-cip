...
* Return Value :  -
******************************************************************************/
void sys_pcie_set_pme_status(struct rzv2m_pcie *pcie, _Bool ast_flag, uint8_t func_no)
{
    if (false != ast_flag)
    {
...
static int pcie_pm_wakeup_l1(struct rzv2m_pcie *pcie, uint8_t fn)
{
	...
	sys_pcie_set_pme_status(pcie, true, fn);
	...
	sys_pcie_set_pme_status(pcie, false, fn);
