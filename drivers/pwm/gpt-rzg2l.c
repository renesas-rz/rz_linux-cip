...
#define RZT2_INT_CCMPA				(1 << 0)
#define RZT2_INT_CCMPB				(1 << 1)
#define RZT2_INT_OVF				(1 << 6)
#define RZT2_NS_INTMSK_OFFSET			0x900
#define RZT2_NS_INTCLR_OFFSET			0x9e0
#define RZT2_NS_INTSTAT_OFFSET			0xac0
#define RZT2_S_INTMSK_OFFSET			0x030
#define RZT2_S_INTCLR_OFFSET			0x038
#define RZT2_S_INTSTAT_OFFSET			0x040
...
static const struct rz_gpt_data_cfg rzt2_cfg = {
		.has_ovf_irq = false,
		.presc = &rzv2h_prescale_info,
};
...
static irqreturn_t rzt2_gpt_isr(int irq, void *data)
{
	...
	if (irq_flags & RZT2_INT_OVF) {
		...
		writew(RZT2_INT_OVF, pc->int_base + pc->intclr_offset
						+ 0x02 * pc->int_offset);
		ret = IRQ_HANDLED;
	}
	...
	if (irq_flags & RZT2_INT_CCMPA) {
		...
		writew(RZT2_INT_CCMPB, pc->int_base + pc->intclr_offset
						+ 0x02 * pc->int_offset);
		ret = IRQ_HANDLED;
	}
	...
	if (irq_flags & RZT2_INT_CCMPB) {
		...
		default:
		 ...
		 /* Disable input capture interrupt flags */
		writew(RZT2_INT_CCMPA, pc->int_base + pc->intclr_offset
						+ 0x02 * pc->int_offset);
		ret = IRQ_HANDLED;
	}
...
static int rzg2l_gpt_probe(struct platform_device *pdev)
{
	...
	rzg2l_gpt->chip = devm_pwmchip_alloc(&pdev->dev, 1, 0); /* no alloc priv data*/
	pwmchip_set_drvdata(rzg2l_gpt->chip, rzg2l_gpt);
	...
	if (rzg2l_gpt->cfg == &rzt2_cfg) {
		...
		if (0 <= gpt_idx_major && 9 >= gpt_idx_major) {
			rzg2l_gpt->int_offset = 5 * gpt_idx_major
								+ gpt_idx_minor;
			rzg2l_gpt->intmsk_offset = RZT2_NS_INTMSK_OFFSET;
			rzg2l_gpt->intclr_offset = RZT2_NS_INTCLR_OFFSET;
			rzg2l_gpt->intstat_offset = RZT2_NS_INTSTAT_OFFSET;
			...
			rzg2l_gpt->intmsk_offset = RZT2_S_INTMSK_OFFSET;
			rzg2l_gpt->intclr_offset = RZT2_S_INTCLR_OFFSET;
			rzg2l_gpt->intstat_offset = RZT2_S_INTSTAT_OFFSET;
			res = platform_get_resource_byname(pdev, IORESOURCE_MEM,
								"s_int_base");
			if (!res) {
				dev_err(&pdev->dev, "missing IO resource\n");
		...
		writew(readw(rzg2l_gpt->int_base + rzg2l_gpt->intmsk_offset
			...
			& ~(RZT2_INT_OVF | RZT2_INT_CCMPA | RZT2_INT_CCMPB),
		...
		ret = devm_request_irq(&pdev->dev, irq, rzt2_gpt_isr, 0,
		...
		rzg2l_gpt->rstc = devm_reset_control_array_get(&pdev->dev, RESET_CONTROL_SHARED);
...
static void rzg2l_gpt_remove(struct platform_device *pdev)
{
...
static const struct of_device_id rzg2l_gpt_of_table[] = {
	...
	{ .compatible = "renesas,gpt-r9a07g076", .data = &rzt2_cfg,},
	{ },
};
