...
static const char *rzg2l_gpt_Z_phases[5] = {
	"NOT_USE",
};
...
struct Z_phase_params {
	struct reg_full gtssr;
	struct reg_full gtpsr;
	struct reg_full gtcsr;
};

static struct Z_phase_params Z_phase_mode_set[5] = {
	[NOT_USE] = {
...
static int rzg2l_gpt_set_Z_phase(struct iio_dev *indio_dev,
				const struct iio_chan_spec *chan,
				unsigned int type)
{
	struct rzg2l_gpt_chip *pc = iio_priv(indio_dev);
	...
	rzg2l_gpt_write(pc, Z_phase_mode_set[type].gtssr.value, GTSSR);
	rzg2l_gpt_write(pc, Z_phase_mode_set[type].gtpsr.value, GTPSR);
	rzg2l_gpt_write(pc, Z_phase_mode_set[type].gtcsr.value, GTCSR);

	return 0;
}

static const struct iio_enum rzg2l_gpt_Z_phase_en = {
	.items = rzg2l_gpt_Z_phases,
	.num_items = ARRAY_SIZE(rzg2l_gpt_Z_phases),
	.get = rzg2l_gpt_get_Z_phase,
	.set = rzg2l_gpt_set_Z_phase,
};
...
static const struct iio_chan_spec_ext_info rzg2l_gpt_cnt_ext_info[] = {
	...
	IIO_ENUM("Z_phase", IIO_SEPARATE,
		&rzg2l_gpt_Z_phase_en),
	IIO_ENUM_AVAILABLE("Z_phase", &rzg2l_gpt_Z_phase_en),
	{}
};
		...
		if (poeg_np != NULL) {
			...
			if (poeg_dev_np) {
				if (strcmp(poeg_np->name, "poega") == 0) {
					rzg2l_gpt_Z_phases[i+1] = "GTETRGA";
					Z_phase_mode_set[i+1] = Z_phase_mode_set_A;
					dev_info(&pdev->dev, "Can use GTETRGA as POEG, Z-phase\n");
				} else if (strcmp(poeg_np->name, "poegb") == 0) {
					rzg2l_gpt_Z_phases[i+1] = "GTETRGB";
					Z_phase_mode_set[i+1] = Z_phase_mode_set_B;
					dev_info(&pdev->dev, "Can use GTETRGB as POEG, Z-phase\n");
				} else if (strcmp(poeg_np->name, "poegc") == 0) {
					rzg2l_gpt_Z_phases[i+1] = "GTETRGC";
					Z_phase_mode_set[i+1] = Z_phase_mode_set_C;
					dev_info(&pdev->dev, "Can use GTETRGC as POEG, Z-phase\n");
				} else if (strcmp(poeg_np->name, "poegd") == 0) {
					rzg2l_gpt_Z_phases[i+1] = "GTETRGD";
					Z_phase_mode_set[i+1] = Z_phase_mode_set_D;
					dev_info(&pdev->dev, "Can use GTETRGD as POEG, Z-phase\n");
				}
			}
		}
	}
