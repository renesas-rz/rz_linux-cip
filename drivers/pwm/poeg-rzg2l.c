...
static void enable_poeg_function(struct rzg2l_poeg_chip *poeg_chip)
{
	...
	rzg2l_poeg_set_bit(poeg_chip, RZG2L_POEGG_PIDE | RZG2L_POEGG_IOCE, RZG2L_POEGG);
	/* Enable noise filter */
	rzg2l_poeg_set_bit(poeg_chip, RZG2L_POEGG_EN_NFEN, RZG2L_POEGG);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!val)
		rzg2l_poeg_clear_bit(poeg_chip, RZG2L_POEGG_SSF, RZG2L_POEGG);
	else
		rzg2l_poeg_set_bit(poeg_chip, RZG2L_POEGG_SSF, RZG2L_POEGG);
		...
		struct device_attribute *attr, char *buf)
{
	...
	read_data = rzg2l_poeg_read(poeg_chip, RZG2L_POEGG);

	if (read_data & RZG2L_POEGG_SSF)
		read_data = 1;
	else
		read_data = 0;
...
static int rzg2l_poeg_probe(struct platform_device *pdev)
{
	...
	ret = reset_control_deassert(poeg_chip->rstc);
	if (ret)
		return dev_err_probe(&pdev->dev, ret, "Cannot deassert reset control\n");
