...
static u8 rz_mtu3_get_prescale(struct rz_mtu3_channel *mtu)
{
	...
	for (i = 0; i < RZ_MTU_NUM_PRESCALES; i++) {
		if (val_tpsc1 == prescale_map[channel_number][i].tpsc1 &&
			val_tpsc2 == prescale_map[channel_number][i].tpsc2) {
			prescale = i;
			break;
		}
	}
	if (prescale == RZ_MTU3_PRESCALE_256)
		prescale = 8;
	else if (prescale == RZ_MTU3_PRESCALE_1024)
		prescale = 10;

	return prescale;
}

static struct rz_mtu3_tcr_tpsc_map rz_mtu3_get_tpsc(unsigned int channel_number, u8 prescale)
{
	if (prescale == 8)
		return prescale_map[channel_number][RZ_MTU3_PRESCALE_256];
	else if (prescale == 10)
		return prescale_map[channel_number][RZ_MTU3_PRESCALE_1024];
	else
			      ...
			      const struct pwm_state *state)
{
	struct rz_mtu3_pwm_chip *rz_mtu3_pwm = to_rz_mtu3_pwm_chip(chip);
	struct rz_mtu3_pwm_channel *priv;
	struct rz_mtu3_tcr_tpsc_map tpsc_map;
	u64 period_cycles;
	u64 duty_cycles;
	u8 prescale;
	u16 pv, dc, deadtime;
...
static int rz_mtu3_pwm_probe(struct platform_device *pdev)
{
	...
	rz_mtu3_pwm->rstc = devm_reset_control_get_optional_shared(pdev->dev.parent, NULL);
