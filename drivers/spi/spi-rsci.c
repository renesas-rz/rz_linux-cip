...
struct spi_ops {
	int (*set_config_register)(struct rsci_data *rsci);
					...
					struct spi_message *msg)
{
	...
	rsci->ops->set_config_register(rsci);
...
static const struct of_device_id rsci_of_match[] = {
	/* RSCI SPI */
	{ .compatible = "renesas,rz-rsci-spi", .data = &rsci_common_ops },
	{ /* sentinel */ }
};
...
static int rsci_spi_parse_dt(struct device *dev, struct spi_controller *ctlr)
{
	...
	if (!IS_ERR(rsci->pinctrl)) {
		...
		return dev_err_probe(dev, PTR_ERR(rsci->pinctrl),
					 "failed to get pinctrl state for %s\n",
					 (rsci->slave)? "slave" : "master");
