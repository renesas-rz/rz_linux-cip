...
#define SPSRC_UDRF		BIT(12)		/* Underrun Error Flag Clear */
...
/*
 *functions for RSPI on T2
 */
static int rspi_t2_set_config_register(struct rspi_data *rspi, int access_size)
{
							...
							u32 enable_bit)
{
	...
	ret = wait_event_timeout(rspi->wait, rspi_read16(rspi, RSPI_SPSR) & wait_mask, 10 * HZ);
	if (!ret)
		return -ETIMEDOUT;
...
static void rspi_t2_receive_init(const struct rspi_data *rspi)
{
...
static int rspi_t2_transfer_one(struct spi_controller *ctlr,
				struct spi_device *spi,
				struct spi_transfer *xfer)
{
	struct rspi_data *rspi = spi_controller_get_devdata(ctlr);

	rspi_t2_receive_init(rspi);
...
static const struct spi_ops rspi_t2_ops = {
	.set_config_register	=	rspi_t2_set_config_register,
	.transfer_one		=	rspi_t2_transfer_one,
	.min_div		=	2,
	.max_div		=	4096,
	.flags			=	SPI_CONTROLLER_MUST_RX | SPI_CONTROLLER_MUST_TX,
	.fifo_size		=	4,	/* 8 for TX, 32 for RX */
	.num_hw_ss		=	1,
};
...
static const struct of_device_id rspi_of_match[] = {
	/* RSPI on T2 */
	{ .compatible = "renesas,rspi-r9a07g076", .data = &rspi_t2_ops },
	{ /* sentinel */ }
};
...
static const struct platform_device_id spi_driver_ids[] = {
	{ "rspi", (kernel_ulong_t)&rspi_t2_ops },
	{},
};
