...
/*
 * Renesas RZT2 TSU Temperature Sensor Unit
 *
...
#define RZT2_TSU_SS_TIMEOUT_US		10000

#define OTP_TSU_REG_ADR_TEMPHI		0x01AB
#define OTP_TSU_REG_ADR_TEMPLO		0x01AC

struct rzt2_thermal_priv {
	struct device *dev;
	void __iomem *base;
...
static int rzt2_thermal_get_temp(struct thermal_zone_device *tz, int *temp)
{
	struct rzt2_thermal_priv *priv = thermal_zone_device_priv(tz);
	u32 result, reg;
	s64 val, val1, val2;
	int ret;
...
static const struct thermal_zone_device_ops rzt2_tz_of_ops = {
	.get_temp = rzt2_thermal_get_temp,
};

static void rzt2_thermal_init(struct rzt2_thermal_priv *priv)
{
	u32 reg;
...
static int rzt2_otp_tsu_get_temp(struct platform_device *pdev)
{
	struct rzt2_thermal_priv *priv = dev_get_drvdata(&pdev->dev);
...
static void rzt2_thermal_remove(struct platform_device *pdev)
{
	struct rzt2_thermal_priv *priv = dev_get_drvdata(&pdev->dev);
...
static int rzt2_thermal_probe(struct platform_device *pdev)
{
	struct thermal_zone_device *zone;
	struct rzt2_thermal_priv *priv;
	...
	rzt2_thermal_init(priv);

	ret = rzt2_otp_tsu_get_temp(pdev);
	if (ret)
		return ret;
...
static const struct of_device_id rzt2_thermal_dt_ids[] = {
	{ .compatible = "renesas,tsu-r9a07g076", },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, rzt2_thermal_dt_ids);

static struct platform_driver rzt2_thermal_driver = {
	.driver = {
		.name = "rzt2_thermal",
		.of_match_table = rzt2_thermal_dt_ids,
	},
	.probe = rzt2_thermal_probe,
	.remove_new = rzt2_thermal_remove,
};
module_platform_driver(rzt2_thermal_driver);

MODULE_DESCRIPTION("Renesas RZ/T2 TSU Thermal Sensor Driver");
