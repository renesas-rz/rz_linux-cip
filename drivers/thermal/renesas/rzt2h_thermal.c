...
static int rzt2h_thermal_get_temp(struct thermal_zone_device *tz, int *temp)
{
	struct rzt2h_thermal_priv *priv = thermal_zone_device_priv(tz);
	u32 result, reg;
	s64 val, val1, val2;
	int ret;
...
static const struct thermal_zone_device_ops rzt2h_tz_of_ops = {
...
static void rzt2h_thermal_remove(struct platform_device *pdev)
{
	struct rzt2h_thermal_priv *priv = dev_get_drvdata(&pdev->dev);
...
static struct platform_driver rzt2h_thermal_driver = {
	...
	.remove_new = rzt2h_thermal_remove,
};
