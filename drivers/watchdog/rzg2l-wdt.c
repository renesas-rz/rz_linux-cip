...
/* Originally defined in /include/linux/units.h */
...
#define WDT_TIMEOUT_REBOOT		0x00770077
...
static int rzg2l_wdt_probe(struct platform_device *pdev)
{
	...
	if(of_property_read_bool(dev->of_node, "timeout-reboot")){
		res = platform_get_resource(pdev, IORESOURCE_MEM, 1);
		if (IS_ERR(res))
			return dev_err_probe(&pdev->dev, PTR_ERR(res), "no cpg base");
		cpg_base = ioremap(res->start, (res->end - res->start) + 1);
		iowrite32(WDT_TIMEOUT_REBOOT, cpg_base + CPG_WDTRST_SEL);
		iounmap(cpg_base);
	}

	/* Get watchdog main clock */
	wdt_clk = of_clk_get(pdev->dev.of_node, 0);
	if (IS_ERR(wdt_clk))
	...
	wdt_clk = of_clk_get(pdev->dev.of_node, 1);
	if (IS_ERR(wdt_clk))
