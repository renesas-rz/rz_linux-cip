...
/*
 * Renesas RZ/T2 WDT Watchdog Driver
 *
...
static u32 rzt2_wdt_get_cycle_usec(struct rzt2_wdt_priv *priv,
						unsigned long cycle,
						u16 wdttime)
{
...
static int rzt2_wdt_ping(struct watchdog_device *wdev)
{
	struct rzt2_wdt_priv *priv = watchdog_get_drvdata(wdev);
	unsigned long delay;
	 ...
	 */
	delay = 4 * rzt2_wdt_get_cycle_usec(priv, priv->pclkl_rate, 0);
	udelay(delay);

	return 0;
}

static void rzt2_wdt_setup(struct watchdog_device *wdev, u16 wdtcr)
{
	struct rzt2_wdt_priv *priv = watchdog_get_drvdata(wdev);
...
static int rzt2_wdt_start(struct watchdog_device *wdev)
{
	struct rzt2_wdt_priv *priv = watchdog_get_drvdata(wdev);
	u32 reg;
	int ret;
	 ...
	 */
	rzt2_wdt_setup(wdev, WDTCR_CKS_CLK_8192 | WDTCR_RPSS_100 |
...
static int rzt2_wdt_stop(struct watchdog_device *wdev)
{
	struct rzt2_wdt_priv *priv = watchdog_get_drvdata(wdev);
	u32 reg;
	int ret;
...
static int rzt2_wdt_init_perierr(struct device *dev, struct device_node *np)
{
...
static const struct watchdog_info rzt2_wdt_ident = {
	.options = WDIOF_MAGICCLOSE | WDIOF_KEEPALIVEPING | WDIOF_SETTIMEOUT,
	.identity = "Renesas RZ/T2 WDT Watchdog",
};

static const struct watchdog_ops rzt2_wdt_ops = {
	.owner = THIS_MODULE,
	.start = rzt2_wdt_start,
	.stop = rzt2_wdt_stop,
	.ping = rzt2_wdt_ping,
};

static int rzt2_wdt_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
	struct rzt2_wdt_priv *priv;
	int ret;

	ret = rzt2_wdt_init_perierr(dev, np);
	if (ret < 0)
		return ret;
	...
	priv->wdev.info = &rzt2_wdt_ident;
	priv->wdev.ops = &rzt2_wdt_ops;
...
static const struct of_device_id rzt2_wdt_ids[] = {
	{ .compatible = "renesas,r9a07g076-wdt", },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, rzt2_wdt_ids);

static struct platform_driver rzt2_wdt_driver = {
	.driver = {
		.name = "rzt2_wdt",
		.of_match_table = rzt2_wdt_ids,
	},
	.probe = rzt2_wdt_probe,
};
module_platform_driver(rzt2_wdt_driver);
MODULE_AUTHOR("Renesas");
MODULE_DESCRIPTION("Renesas RZ/T2 WDT Watchdog Driver");
