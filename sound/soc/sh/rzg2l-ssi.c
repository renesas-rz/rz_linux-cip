...
/*
 * Serial Sound Interface (SSIF) support for RZ/G2L SoCs
...
static void ssi_dma_complete(void *data)
{
	...
	if (strm->running) {
		 ...
		 * started
		 */
		ssi_pointer_update(strm, strm->substream->runtime->period_size);
...
static const struct of_device_id ssi_of_match[] = {
	{ .compatible = "renesas,rzg2l-ssi", .data = &ssi2_data },
	{},
};
